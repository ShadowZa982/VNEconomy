...
public class EconomyAPI {
    ...
    // Sync convenience (backed by storage implementation)
    public static double getBalance(UUID uuid) { return provider.getBalance(uuid); }
    public static boolean has(UUID uuid, double amount) { return provider.getBalance(uuid) >= amount; }
    public static void set(UUID uuid, double amount) {
        ...
        // Sau khi gọi event -> mới set (nếu không ai cancel, bạn có thể mở rộng sau)
    ...
    // Async variants
    public static CompletableFuture<Double> getBalanceAsync(UUID uuid) { return provider.getBalanceAsync(uuid); }
    public static CompletableFuture<Void> setAsync(UUID uuid, double amount) { return provider.setAsync(uuid, amount); }
    public static CompletableFuture<Void> giveAsync(UUID uuid, double amount) { return provider.giveAsync(uuid, amount); }
    public static CompletableFuture<Boolean> takeAsync(UUID uuid, double amount) { return provider.takeAsync(uuid, amount); }
}
